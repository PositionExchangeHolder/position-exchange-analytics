import { NftPoolBalance, StakingPoolBalances } from 'api/address/address.api.type'
...
const StakingItem = ({ id, name, stakingBalance, pendingReward }: StakingItemProps) => {
  return (
...
export default function StakingListItem({ stakingPoolBalances, nftPoolBalance }: StakingListItemProps) {
  return (
    <div className="flex flex-col m-auto mt-12 bg-secondary rounded-md">
      <div className="flex overflow-x-scroll py-10 scrollbar-hide">
        <div className="flex flex-nowrap md:ml-10 lg:ml-20">
          {
            nftPoolBalance
              && isValidStakingPool(nftPoolBalance?.stakingBalance, nftPoolBalance?.pendingReward)
              && <StakingItem
